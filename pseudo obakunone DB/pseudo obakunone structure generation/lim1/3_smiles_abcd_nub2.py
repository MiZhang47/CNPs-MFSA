...
for i, row_a in df_a.iterrows():
    for j, row_b in df_b.iterrows():
        try:
            ...
            mol_ab = fuse_by_common_edge(mol_a, mol_b, map_a, map_b, 2, 3)
            mol_ab = mark_colored_atoms(mol_ab, color_tag)

            fused_smiles = Chem.MolToSmiles(mol_ab, isomericSmiles=True)
            b_smiles_colored = Chem.MolToSmiles(mol_b)

            fused_ab_data.append({
                "A_Label": row_a.get("Label", f"A{i}"),
                "B_Label": row_b.get("Label", f"B{j}"),
                "A_SMILES": smi_a,
                "A_Atom_Indices": row_a['Ring_Atom_Indices'],
                "A_Assigned_Numbers": row_a['Ring_Assigned_Numbers'],
                "B_SMILES": b_smiles_colored,
                "B_Atom_Indices": row_b['Ring_Atom_Indices'],
                "B_Assigned_Numbers": row_b['Ring_Assigned_Numbers'],
...
for i, row_c in df_c.iterrows():
    for j, row_d in df_d.iterrows():
        try:
            ...
            c_smiles_colored = Chem.MolToSmiles(mol_c)

            fused_cd_data.append({
                "C_Label": row_c.get("Label", f"C{i}"),
                "D_Label": row_d.get("Label", f"D{j}"),
                "C_SMILES": c_smiles_colored,
                "C_Atom_Indices": row_c['Ring_Atom_Indices'],
                "C_Assigned_Numbers": row_c['Ring_Assigned_Numbers'],
                "D_SMILES": smi_d,
                "D_Atom_Indices": row_d['Ring_Atom_Indices'],
                "D_Assigned_Numbers": row_d['Ring_Assigned_Numbers'],
