...
import dash_bootstrap_components as dbc  #qq Bootstrap-based UI components for Dash
...
app.layout = html.Div([  # Main container for the application
    ...
    dcc.Tabs([  # Create a set of tabs for different functionalities
        ...
        dcc.Tab(label='Modules Annotation-Daphnane', children=[
            html.Div([
                # --- Store to hold the final pseudo-DB absolute path (from upload or local path) ---
                dcc.Store(id='pseudo-db-path-store'),
                ...
                html.Div([
                    ...
                    html.Div([
                        html.H4('Import pseudo-DB file (for small files):', style={'fontFamily': 'Arial'}),
                        dcc.Upload(
                        ...
                        # === New: Local path input box + button + status ===
                        html.Div("Or specify local DB path (for large files):",
                                 style={'marginTop': '15px', 'fontFamily': 'Arial', 'fontWeight': 'bold'}),
                        dcc.Input(id='db-path-input', type='text',
                                  placeholder='C:\\path\\to\\pseudo-D.db',
                                  style={'width': '80%', 'fontFamily': 'Arial'}),
                        html.Button('Load DB (local path)', id='load-db-btn', n_clicks=0,
                                    style={'marginLeft': '8px', 'fontFamily': 'Arial'}),
                        html.Div(id='db-path-status',
                                 style={'marginTop': '6px', 'fontFamily': 'Arial', 'color': '#444'})
                    ], style={'width': '33%'}),

                    # Upload section for the Substituent CSV file (multiple files)
                    html.Div([
                        html.H4('Import Substituent CSV file:', style={'fontFamily': 'Arial'}),
                        dcc.Upload(
                ...
                html.A("Download All Results", id="annotation-download-link",
                ...
                html.Div([
                    html.Div([
                        html.H4('Select file to view results:',
                                style={'fontFamily': 'Arial', 'marginRight': '20px'}),
                        dcc.Dropdown(
                            ...
                            style={'width': '200px', 'fontFamily': 'Arial'}
                              ...
                              'fontFamily': 'Arial', 'marginTop': '20px'}),
                    html.Div(id='annotation-results-table',
                             style={'marginTop': '20px', 'fontFamily': 'Arial'})
                ])
            ])
        ], style={
            'border': '2px solid #d6d6d6',
...
@app.callback(
    [
        ...
        Output('pseudo-db-path-store', 'data', allow_duplicate=True)  # 关键！
    ],
...
def update_annotation_pseudo_DB_file_path(content, filename):
    ...
    if content and filename:
        saved_file = save_uploaded_file(filename, content)  # 请确保返回绝对路径
        if saved_file:
            return f"Uploaded pseudo-DB file: {filename}", saved_file
        else:
            return "Failed to upload the file.", None
    return "No DB file uploaded", None

@app.callback(
    [
        Output("db-path-status", "children"),
        Output("pseudo-db-path-store", "data")  # 这个保持默认（不需要重复允许）
    ],
    Input("load-db-btn", "n_clicks"),
    State("db-path-input", "value"),
    prevent_initial_call=True
)
def use_local_db(n_clicks, path):
    if not n_clicks:
        raise PreventUpdate
    if not path or not os.path.exists(path):
        return "❌ Invalid path or file does not exist", None
    try:
        if path.lower().endswith((".db", ".sqlite", ".sqlite3")):
            with sqlite3.connect(path) as conn:
                conn.execute("SELECT 1")
        return f"✅ Using local DB: {path}", path
    except Exception as e:
        return f"❌ Failed to open DB: {e}", None
...
@app.callback(
    ...
    State('annotation-query-upload-csv', 'filename'),
    State('annotation-substituent-upload-csv', 'filename'),
    State('pseudo-db-path-store', 'data'),   # unified pseudo-DB path (upload or local)
)
def run_annotation(n_clicks, query_fns, acyl_fns, pseudo_db_path):
    if not n_clicks:
        return "Click Run Annotation to start processing."

    # --- check inputs ---
    if not query_fns:
        return "❌ No query CSV file provided."
    if not acyl_fns:
        return "❌ No substituent CSV file provided."
    if not pseudo_db_path:
        return "❌ No pseudo-DB selected. Please upload a small DB or use local path mode."
    ...
    # construct file paths for CSV and substituent (they are still uploaded into UPLOAD_DIR)
    query_path = os.path.join(UPLOAD_DIR, 'tab_7_uploads', query_file)
    acyl_path  = os.path.join(UPLOAD_DIR, 'tab_7_uploads', acyl_file)

    # pseudo_db_path is already absolute (from upload save or local path input)
    pseudo_path = pseudo_db_path
